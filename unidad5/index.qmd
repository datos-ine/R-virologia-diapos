---
pagetitle: "Estadística descriptiva"
subtitle: "Unidad 5"
format: 
  revealjs:
    auto-stretch: false
...
##  {#intro-curso data-menu-title="Estadística descriptiva" .invert}


[**Estadística descriptiva**]{.custom-title} 

[***Unidad 5***]{.custom-subtitle}

## Estadísticos descriptivos de R base{.title-top}
...
Tomemos la siguiente tabla de datos ficticios de resultados de laboratorio (mostramos las primeras 4 observaciones):
...
datos <- read_csv2("datos/ENT.txt")

ENT <- datos |> select(ID, Sexo, Colesterol, Glucemia, Trigliceridos) |> na.omit()
...
ENT |> summarise(
  ...
  meda_glucemia = mean(Glucemia),
  media_trigliceridos = mean(Trigliceridos)
)
```
...
ENT  |> summarise(
  across(.cols = Colesterol:Trigliceridos, .fns = mean))
...
#| echo: false
datos <- datos |> 
  select(ID, Sexo, Colesterol, Glucemia, Trigliceridos) |> 
  arrange(Colesterol) 
...
datos |> 
  summarise(
    across(Colesterol:Trigliceridos, mean)
  )
```
...
datos |> 
  summarise(
    across(Colesterol:Trigliceridos, \(x) mean(x, na.rm = TRUE)))
```
...
datos |> 
  summarise(
    across(Colesterol:Trigliceridos, list(
...
Aplicamos la función `log()` para calcular el logaritmo de los valores de las todas las variables.
...
datos |> 
  mutate(
    across(Colesterol:Trigliceridos, log)) |> 
  slice_head(n = 4) # vemos las primera 4 observaciones
```
...
datos |> 
  filter(if_any(Colesterol:Trigliceridos, is.na))
```
<br>
...
datos |> 
  filter(if_all(Colesterol:Trigliceridos, is.na))
```
...
datos |> 
  filter(if_all(Colesterol:Trigliceridos, \(x) x > 140 & x < 200))
```
...
Estas tablas son convenientes para reportes, publicaciones y presentaciones, asegurando que la información clave sea fácilmente comprensible.
...
![](images/gtsummary.PNG){fig-align="center" width="1400"}
...
- `tbl_summary()`:  Calcula estadísticas descriptivas para variables continuas, categóricas y dicotómicas.

- `tbl_regression()`: Convierte un objeto de modelo de regresión en una tabla formateada y personalizada.

- `tbl_survfit()`: Convierte un objeto survfit (supervivencia) en una tabla personalizada con estimaciones de tiempo hasta el evento.

## gtsummary {.title-top}

<br>

En la función del ejemplo anterior `tbl_summary()`, se puede personalizar la salida con:

::: incremental

- Variable categórica de estratificación/agrupamiento
- Etiqueta de variables
- Estadísticos a mostrar en variables cuantitativas y cualitativas
- Definir cantidad de digitos en redondeo
- Valores ausentes: no tenerlos en cuenta o cambiarle la etiqueta 
- Ordenamientos
- Agregar valores p de test estadísticos entre grupos
- etc.
...
datos |> 
  freq_table(Sexo) |> 
  flextable() |> # convertir a flextable
  fontsize(size = 30, part = "all") |>  # modificar tamaño de fuente
  align(align = "center", part = "all") |>  # alinear textos
  line_spacing(space = 2, part = "all") |> # espacios en filas y columnas
  padding(padding = 6, part = "header") |> # espacio en titulo
  set_header_labels(n = "Frecuencia", prop = "%") |> # cambiar cabecera
  theme_zebra() # agregar tema tipo zebra
```
